...
Java_com_example_echo_EchoActivity_createSLEngine(JNIEnv *env, jobject thiz, jint rate,
    ...
    engine.fastPathSampleRate_ = static_cast<SLmilliHertz>(rate) * 1000;
...
Java_com_example_echo_EchoActivity_configureEcho(JNIEnv *env, jobject thiz, jint delayInMs,
                                                 jfloat decay) {
    engine.echoDelay_ = delayInMs;
    engine.echoDecay_ = decay;
...
Java_com_example_echo_EchoActivity_createAudioRecorder(JNIEnv *env, jobject thiz) {
    SampleFormat sampleFormat;
    memset(&sampleFormat, 0, sizeof(sampleFormat));
    sampleFormat.pcmFormat_ = static_cast<__uint16_t>(engine.bitsPerSample_);
// SampleFormat.representation_ = SL_ANDROID_PCM_REPRESENTATION_SIGNED_INT;
